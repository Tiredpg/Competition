...
void Encoder_Init(void){
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3,ENABLE);//开启时钟5，时钟1输出PWM
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);
  
	GPIO_InitTypeDef GPIO_InitStructure1;
	GPIO_InitStructure1.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;	//端口配置 PA0,PA1
	GPIO_InitStructure1.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure1.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
	GPIO_Init(GPIOA, &GPIO_InitStructure1);					      //根据设定参数初始化GPIOB
	
	
	TIM_InternalClockConfig(TIM3);//选择内部时钟驱动时基单元，不写也有默认

	TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure1;
	TIM_TimeBaseInitStructure1.TIM_ClockDivision = TIM_CKD_DIV1;//1分频，不分频，滤波器的采样频率
	TIM_TimeBaseInitStructure1.TIM_CounterMode=TIM_CounterMode_Up ;//向上计数
	TIM_TimeBaseInitStructure1.TIM_Period=65536-1;//自动重装器，越大计数越多,ARR
	TIM_TimeBaseInitStructure1.TIM_Prescaler=1-1;//预分频器，越小频率越高,PSC,1KHZ
	TIM_TimeBaseInitStructure1.TIM_RepetitionCounter=0;//高级计时器才用
	TIM_TimeBaseInit(TIM3,&TIM_TimeBaseInitStructure1);//时基单元初始化，更新中断和更新事件同时发生
	TIM_ClearFlag(TIM3, TIM_FLAG_Update);//手动清除中断标志位，避免刚初始化就进中断

	TIM_ICInitTypeDef TIM_ICInitStructure1;
	TIM_ICStructInit(& TIM_ICInitStructure1);//结构体赋默认值
	TIM_ICInitStructure1.TIM_Channel = TIM_Channel_1 ;
	TIM_ICInitStructure1.TIM_ICFilter = 0xF;//配置滤波器，其采样频率不改变信号频率
	TIM_ICInitStructure1.TIM_ICSelection= TIM_ICPolarity_BothEdge;//上升沿下降沿有效
	TIM_ICInitStructure1.TIM_ICPolarity = TIM_ICPolarity_Rising;//高低电平极性翻转
